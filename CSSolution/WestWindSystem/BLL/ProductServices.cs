...
namespace WestWindSystem.BLL
{
    public class ProductServices
    {
        ...
        #region CRUD Add, Update and Delete services
        ...
        public int Product_Add(Product item)
        {
            ...
            //to test for corrupt data 
            //COMMENT OUT AFTER DOING THE TEST!!!!!!!!!!!!!!!!!!
            //item.UnitPrice = -4.59m;
        ...
        public int Product_LogicalDelete(Product item)
        {
            ...
            //  was data actually passed to the method
            if (item == null)
                throw new ArgumentNullException("Product information was not received. Remove not done.");
            ...
            //  b) return a null
            Product exists = null;
            //retreive the current record for the product id off the database
            exists = _context.Products
                            .FirstOrDefault(x => x.ProductID == item.ProductID);
            //test that a record does exist
            if (exists == null)
                throw new ArgumentException($"Product {item.ProductName}  " +
                   $"  of size {item.QuantityPerUnit} is not on file. Check for the product again.");
        ...
        public int Product_PhysicalDelete(Product item)
        {
            ...
            if(_context.Products.Any(x => x.ManifestItems.Count > 0))
                throw new ArgumentException($"Product {item.ProductName}  " +
                  $"  of size {item.QuantityPerUnit} has manifest associated records. Cannot remove product from database.");

            if (_context.Products.Any(x => x.OrderDetails.Count > 0))
                throw new ArgumentException($"Product {item.ProductName}  " +
                  $"  of size {item.QuantityPerUnit} has order detail associated records. Cannot remove product from database.");
        ...
        public int Product_Activate(Product item)
        {
            ...
            //for the ativation
            //  set the appropriate field to the value indicating "active product"
            //this code is not relying on the user to have set the appropriate
            //  field on the form
            //  note: no OTHER field on the current record is altered
